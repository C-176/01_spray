...
# 筛选包含信息的标签
img_a = page_text.find_all('a', class_="HotItem-img")
# 创建列表存放标题与网址
img_list = []
title_list = []
# 遍历并正则获得标题与地址
for i in img_a:
    img_url = re.findall(pattern, str(i))[0]
    title = str(re.findall(pattern1, str(i))[0]).replace('?', '？').replace('/', '或')
...
# 遍历获取图片并存文件
for index in tqdm(range(len(img_list)), colour='BLUE'):
    img_response = requests.get(img_list[index], headers=headers).content
    # print(title_list[index])
    title_text = title_list[index]+'.jpg'
    with open(r'./知乎热榜图片/%s' % title_text, 'wb') as file:
